import { Component, computed, Directive, EventEmitter, Input, input, Output } from '@angular/core';
import { RdxCheckboxInputDirective } from '@radix-ng/primitives/checkbox';
import {
...
@Component({
    ...
    template: `
        <button [checked]="shChecked" [class]="computedClass()" (click)="onClick()" rdxDropdownMenuItemCheckbox>
            <span class="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
                <div rdxDropdownMenuItemIndicator>
                    <lucide-icon class="h-4 w-4" size="16" name="check"></lucide-icon>
                </div>
            </span>
...
export class DropdownMenuCheckboxItemComponent {
    ...
    onClick() {
        this.shChecked = !this.shChecked;
        this.checkedChange.emit(this.shChecked);
    }
}
