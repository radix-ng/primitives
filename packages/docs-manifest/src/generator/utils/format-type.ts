import { ArrayType, DeclarationReflection, Reflection, ReflectionType, SomeType, UnionType } from 'typedoc';
...
function needsCustomFormatting(type: SomeType): boolean {
    return !!(
        ...
        (type instanceof UnionType &&
            type.types.find(
                (t) =>
                    t instanceof ReflectionType || (t instanceof ArrayType && t.elementType instanceof ReflectionType)
            )) ||
...
export function formatType(
    ...
    if (!type && reflection instanceof DeclarationReflection) {
        type = reflection.type ?? reflection.getAllSignatures().find((signature) => signature.type)?.type;
    }
...
`
    ...
    formatted = remapLambdaNames(formatted, getNearestProjectReflection(reflection));
