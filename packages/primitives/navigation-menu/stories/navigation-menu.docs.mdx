...
`data-starting-style` / `data-ending-style` and owns `--popup-width` / `--popup-height`. `Arrow`
reflects `data-open` / `data-closed` / `data-side` / `data-align` / `data-uncentered` (set when it
cannot be centered on the anchor — it stays visible rather than disappearing). `Backdrop`
