...
keyframes on the first DOM element inside its template. The popup's resting transform should read the
swipe variables so the gesture and snap-back are visible:
