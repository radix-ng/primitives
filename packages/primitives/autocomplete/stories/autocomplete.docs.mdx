...
`ItemIndicator`, `Empty`, `Status`, `Trigger`, `Icon`, `Clear`, `Label`, `Anchor`, `InputGroup`,
`Arrow` (re-exposes `width` / `height`; carries `data-uncentered` when it can no longer be centered on
the anchor — it stays visible rather than disappearing)) read everything from context and take no
inputs of their own.
