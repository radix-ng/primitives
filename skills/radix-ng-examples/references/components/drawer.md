...
`RdxDrawerVirtualKeyboardProvider` is a no-input directive for the drawer viewport. It exposes
`--drawer-keyboard-inset`, aligns focused keyboard inputs above the virtual keyboard, applies and
restores scroll slack, uses touch tap-to-focus for iOS keyboard opening, and suspends itself while a
nested drawer is open.
