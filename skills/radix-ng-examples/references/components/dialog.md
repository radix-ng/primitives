...
| ----------------- | ---------------------------------------------------------- |
| `data-popup-open` | This trigger owns the currently open dialog.               |
| `disabled`        | The trigger is disabled.                                   |

`aria-controls` is set only while this trigger owns the open dialog (it points at the popup's id, which exists only then); `aria-expanded` reflects the same ownership; `aria-haspopup` is `"dialog"`.
...
| `data-closed`             | The dialog is closed.                                             |
| `data-starting-style`     | The enter transition is about to run.                            |
| `data-ending-style`       | The exit transition is running.                                  |
| `data-nested`             | This dialog is nested inside another dialog.                     |
| `data-nested-dialog-open` | A dialog nested inside this one is open.                         |
...
The popup carries `role="dialog"` (`alertdialog` for the alert-dialog variant); `aria-modal="true"` is set only for a fully modal dialog; `aria-labelledby` / `aria-describedby` are wired automatically when `rdxDialogTitle` / `rdxDialogDescription` are present.

**CSS variables**

| Variable           | Description                                                  |
| ------------------ | ----------------------------------------------------------- |
| `--nested-dialogs` | Number of open dialogs nested directly inside this one, for depth-based stacking effects. |

### Backdrop

`RdxDialogBackdrop` is a decorative overlay (`role="presentation"`). A nested dialog renders no backdrop
by default — set `forceRender` to opt back in.
...
`Close` is a native `button` that closes the dialog on click.

## Accessibility

Dialog uses native HTML where it can (the trigger and close are real
`<button>`s) and adds ARIA only for the windowing semantics the platform does
not provide. It is built against the
[APG Dialog (Modal) Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/).

| Area      | Implementation                                                                                  | Reference                                                                                                              |
| --------- | ----------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Semantics | `role="dialog"`, `aria-modal="true"` (modal only), labelled by `Title`, described by `Description` | [APG Dialog](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/), [WCAG 4.1.2](https://www.w3.org/TR/WCAG22/#name-role-value) |
| Keyboard  | `Escape` closes the topmost dialog; `Tab` / `Shift+Tab` cycle focus within a modal dialog        | [APG Dialog Keyboard](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/#keyboardinteraction), [WCAG 2.1.1](https://www.w3.org/TR/WCAG22/#keyboard) |
| Focus     | Initial focus moves into the dialog on open; focus is trapped while modal; focus is restored to the trigger on close | [WCAG 2.1.2 No Keyboard Trap](https://www.w3.org/TR/WCAG22/#no-keyboard-trap), [WCAG 2.4.3 Focus Order](https://www.w3.org/TR/WCAG22/#focus-order) |
| State     | `aria-controls` links the trigger to the open popup; `aria-expanded` reflects open state; `data-open` / `data-closed` for styling | [WAI-ARIA states and properties](https://www.w3.org/TR/wai-aria-1.2/#states_and_properties) |

A non-modal dialog (`modal="trap-focus"` or `modal="false"`) intentionally drops
`aria-modal` and does not render content underneath inert, so `Tab` can leave it
— see the [Non-modal](#non-modal) and [Trap focus only](#trap-focus-only)
examples.
