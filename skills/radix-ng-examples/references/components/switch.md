...
`RdxSwitchInput` — the hidden native checkbox for form submission. Apply to an `<input>` element inside
the root. Reads everything from context (carries `name` / `value`); it has no inputs or `data-*`.
