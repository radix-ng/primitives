...
```html
<div class="flex w-full max-w-xl gap-4" rdxStepperRoot [value]="2">
  ...
  >
    ...
    <div class="text-muted-foreground mt-2 w-full text-center">
      <h4 class="text-foreground text-sm font-medium">{{ item.title }}</h4>
...
```html
<div class="flex w-full max-w-sm flex-col gap-5" rdxStepperRoot [value]="2" orientation="vertical">
  @for (item of steps; track $index) {
  <div
  ...
  >
    @if (item.step !== steps[steps.length - 1].step) {
    <div class="bg-border absolute top-10 -bottom-5 left-5 w-px -translate-x-1/2" rdxStepperSeparator></div>
    }
    <button
