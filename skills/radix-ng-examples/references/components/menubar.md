...
up arrow-key navigation and hover-to-switch behavior between them.
...
| Key                                 | Behavior                                                            |
| ----------------------------------- | ------------------------------------------------------------------- |
| <kbd>ArrowDown</kbd>                | Opens the focused trigger's menu and moves focus to the first item. |
| <kbd>ArrowUp</kbd>                  | Opens the focused trigger's menu and moves focus to the last item.  |
| <kbd>ArrowRight</kbd>               | Moves to the next trigger. Opens it if a menu was already open.     |
| <kbd>ArrowLeft</kbd>                | Moves to the previous trigger. Opens it if a menu was already open. |
| <kbd>Home</kbd>                     | Moves to the first trigger.                                         |
| <kbd>End</kbd>                      | Moves to the last trigger.                                          |
| <kbd>Escape</kbd>                   | Closes the open menu and returns focus to its trigger.              |
| <kbd>Enter</kbd> / <kbd>Space</kbd> | Activates the focused item (handled by the Menu primitive).         |

Triggers move with a roving tabindex: only one trigger is in the tab order at a time, and
...
menu of `rdxMenubarRoot`, the menubar wires it for roving focus, hover switching, and ArrowLeft /
...
stays keyboard-navigable. It also reports `role="menuitem"` and is taken out of the tab order
(`tabindex="-1"`) so the whole bar acts as a single tab stop.
