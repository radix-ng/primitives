...
- ✅ Mirrors classic `NgControl` dirty/touched state — including reset, pristine, and untouched
  transitions — on every dual Reactive/template-driven control.
...
`NgControl state` means the control reflects `dirty` / `touched` and reacts to Angular's default
`reset()`, `markAsPristine()`, and `markAsUntouched()` lifecycle. It applies to both Reactive Forms and
`ngModel`; Signal-only controls use `FieldState.reset()` instead.

| Control        | Signal value                      | `[formField]` | Reactive / `ngModel` | `NgControl` state | `FieldState.reset()` |
| -------------- | --------------------------------- | ------------- | -------------------- | ----------------- | -------------------- |
...
- **`RdxFormUiControlBase`** — an abstract `@Directive()` that declares the six state members **once** and
  ...
  `touched` flags as authoritative and follows their reset / pristine / untouched transitions.
