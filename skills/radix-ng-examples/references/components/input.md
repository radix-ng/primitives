  ...
  template: `
    <form class="flex w-[24rem] flex-col gap-5" [formGroup]="form" (ngSubmit)="submit()">
      ...
      >
        ...
        />
        <p class="text-muted-foreground text-sm" rdxFieldDescription>Used for product updates and account recovery.</p>
        <p class="text-destructive text-sm" rdxFieldError>Enter a valid email address.</p>
      </div>
      ...
      >
        ...
        <input [class]="inputClass" rdxInput type="password" autocomplete="new-password" formControlName="password" />
