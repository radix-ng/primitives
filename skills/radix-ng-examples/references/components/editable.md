...
`onValueChange` emits `{ value, eventDetails }` when the edited value is submitted. Call
`eventDetails.cancel()` to keep the editor open and preserve the previous committed value.

```html
<div [value]="value()" (onValueChange)="submitValue($event)" rdxEditableRoot>
    ...
</div>
```

```ts
submitValue(change: RdxEditableValueChangeEvent) {
