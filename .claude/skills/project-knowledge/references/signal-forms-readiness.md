...
| **number-field-root** (`number-field/src/number-field-root.directive.ts`) | `FormValueControl<number>`             | `value = model<number>` ✅    | `disabled`, `readonly`, `required`, `min`, `max` | `invalid`, `errors`, `touched`, `dirty`, `name`                                 | 🟢 `min/max` already align — **`implements RdxFormValueControl<number \| undefined>` ✅** (compiles)                                                                                                            |
