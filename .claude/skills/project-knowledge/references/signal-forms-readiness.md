...
Cross-control conformance specs now mount every one of the 14 controls with
`[formField] rdxSignalField` and every one of the 11 dual controls with
`formControlName rdxNgControlField` on its real root host. This verifies the
complete Field presentation lifecycle rather than inferring it from the generic
adapter implementation.

Angular's `MaybeFieldTree` gives `undefined` structural meaning: an object
property with that value is an absent optional child, not a bindable leaf.
Signal Forms models therefore use concrete empty values (`''`, `[]`, or `null`).
Date Field and Time Field now expose `DateValue | null` / `TimeValue | null`; the
breaking `undefined` → `null` empty sentinel is recorded in ADR 0022.
...
| **date-field-root** (`date-field/src/date-field-root.directive.ts`) | `FormValueControl<DateValue \| null>`     | `value = model` ✅                 | `disabled`, `readonly`, `invalid`, `errors`, `touched` model + `touch`, `dirty`                                                                                        | `required`, `name`, `min/max`                                      | 🟢 **batch #4 DONE — `extends RdxFormUiControlBase`, `implements RdxFormValueControl<DateValue \| null>` ✅**. **No CVA** (Signal-Forms-only). `null` is the empty Signal Forms leaf; `undefined` would remove the child `FieldTree`. `invalidState` combines built-in range/availability checks with form-driven invalidity; state reflects on root + segments.                                                                                                                                                                                                     |
| **time-field-root** (`time-field/src/time-field-root.directive.ts`) | `FormValueControl<TimeValue \| null>`     | `value = model` ✅                 | `disabled`, `readonly`, `invalid`, `errors`, `touched` model + `touch`, `dirty`                                                                                        | `required`, `name`, `min/max`                                      | 🟢 **batch #4 DONE — same as date-field** (`extends RdxFormUiControlBase`, `implements RdxFormValueControl<TimeValue \| null>`, no CVA). `null` is the empty Signal Forms leaf; segment editing maps its empty buffer back to `null`.                                                                                                                                                                                                                                                                                                                                |
