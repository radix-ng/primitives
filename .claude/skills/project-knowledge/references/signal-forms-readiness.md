...
| **switch-root** (`switch/src/switch-root.directive.ts`)                   | `FormCheckboxControl`                | `checked = model<boolean>` ✅ | `disabled`, `required`                           | `readonly`, `invalid`, `errors`, `touched`, `dirty`                                         | 🟢 clean                                                                                                                                            |
...
6. **Remove CDK from the form path** — `BooleanInput` from `@angular/cdk/coercion`
   (`control-value-accessor.ts:1`, `field-control`, etc.) and `_IdGenerator`
   (`switch-root.directive.ts:75`). Pure prep, aligns with the documented CDK
   phase-out, shrinks the upgrade surface.
