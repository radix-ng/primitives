...
| **select-root** (`select/src/select-root.ts`)                       | `FormValueControl<AcceptableValue…>`      | `value = model` ✅                 | `disabled`, `required`, `readOnly`, `invalid`, `errors`, `touched` model + `touch`, `dirty`                                                                            | `name` (no native form submission), `readonly` casing (`readOnly`)      | 🟢 **batch #4 DONE — `implements RdxFormValueControl<AcceptableValue \| AcceptableValue[] \| undefined>` ✅** (compiles). **No CVA** (unlike switch/number-field): Signal Forms binds via custom-control discovery of the `value` model; Reactive/template-driven binding would need a future `ControlValueAccessor`. State lives on the **trigger** (the combobox button), OR-ed from the root context + Field context; `markAsTouched()` (model + `touch`, no CVA call) fires on trigger blur; `dirty` tracked in `setValue` |
...
- 🟢 **select — batch #4 DONE** (optional `FormUiControl` surface: `invalid` / `errors` /
  `touched` model + `touch` / `dirty`, reflected as `data-*` + `aria-invalid` on the **trigger**;
  Signal Forms value binding verified in `select-attributes.spec.ts`). **Caveat:** select carries
  **no `ControlValueAccessor`**, so it is _not_ dual — Signal Forms binds via custom-control
  discovery of the `value` model, but Reactive/template-driven forms have no binding until a CVA is
  added. `markAsTouched()` (no CVA call) fires on trigger blur; `dirty` tracked in `setValue`.
...
- 🟡 **editable** — needs `invalid/errors/touched/dirty` (+ `required/name` where missing). Extend `RdxFormUiControlBase` (or call `createFormUiState` if already extending another base).
...
date-field, time-field** all `extends RdxFormUiControlBase` (compodoc resolves the inherited inputs, so
ArgTypes/api-contract are intact). slider extends the base but skips the `implements` check (union value
vs scalar `min`/`max`).
