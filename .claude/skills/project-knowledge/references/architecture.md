...
- **Form is the top of the forms layer cake (Base UI `Form`)** — `form` (`form[rdxFormRoot]`, its own secondary entry `@radix-ng/primitives/form`) rides the native `<form>`: maps server/external `errors` onto fields by `name` (clearing on user edit, emitting `onClearErrors`), intercepts submit (`onFormSubmit` with `FormData`-serialized values, focus the first invalid field), resets field interaction state on native reset, and exposes aggregate `data-invalid`/`data-dirty`/`data-touched`/`data-submitting`. It owns no values or validation and never `stopPropagation`s, so Reactive Forms' `(ngSubmit)` still fires alongside. **Dependency direction is `field → form`** (Field optionally injects the Form context and registers through structural `() =>` accessors; `form` imports nothing from `field`) — the same acyclic cross-entry constraint as the checkbox/checkbox-group co-location, solved here by keeping the registration interface Form-side. Field gained a `name` input, external-error merge into `invalidState`, edit-forwarding via bubbling input/change events, and `RdxFieldError.messages()`; `RdxFieldState` gained an optional `errors` accessor. A form-level `RdxFormState` provider seam (mirroring Field's `RdxFieldState`) keeps the future Signal Forms `[rdxSignalForm]` adapter migration adapter-only (see `signal-forms-readiness.md`). No new runtime peer — the new edge is internal cross-entry only.
- **Combobox supports external list virtualization** — an opt-in `virtualized` mode where the root takes the full dataset as `items` and navigates/filters by **index** over that data instead of over DOM-registered item directives (only a window is mounted). `filteredItems()` exposes the matching subset for an external virtualizer — the library ships none, consumers wire one (e.g. `@tanstack/angular-virtual`), so it is **not** a peer dependency. `onItemHighlighted` emits `{ value, index, reason }` so the consumer can `scrollToIndex` before the row mounts; each rendered item carries an `[index]`. The non-virtualized DOM-registration path is unchanged (additive second mode behind `virtualized()` branches). Object item values require `itemToStringLabel` for correct selection labels (the root warns in dev otherwise). Select is intentionally not virtualized (upstream Base UI keyboard-nav limitation).
