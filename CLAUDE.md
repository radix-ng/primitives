...
- The generated bundle under `skills/` powers two consumer Agent Skills (`radix-ng`, `radix-ng-examples`) distributed via skills.sh (`npx skills add radix-ng/primitives/skills`), structured for **progressive disclosure**. `radix-ng` ships **per-primitive** contract shards — `references/api-contract/<slug>.json` (selector, `exportAs`, inputs/outputs, two-way bindings) and `references/styling-contract/<slug>.json` (parts + `data-*`) — plus deprecated whole-library monoliths kept one release, and a "Recovering after context loss" protocol in `SKILL.md`. `radix-ng-examples` ships a compact, budget-capped `SKILL.md` router, per-component **index** docs (`references/components/<slug>.md`) that link to self-contained per-example shards (`references/examples/<slug>--<example>.md`), plus `llms.txt` and `llms-full.txt`. Every file is version-stamped. Full pipeline: `architecture.md`.
- Generator: `tools/scripts/skills/generate.mjs` (+ `storybook-docs.mjs`, an Astro-independent renderer that resolves `<Canvas>` blocks into real source; + `api-contract.mjs`, which reads the Storybook compodoc `documentation.json` — regenerating it via compodoc automatically when missing or older than `packages/primitives` sources). Run `pnpm skills:build` after changing any `*.docs.mdx` **or any primitive's public API**. It throws (failing CI) if an **eager-load budget** or an **integrity guard** is breached (version stamp, component ↔ contract ↔ example correspondence, orphan shards, broken relative links, public-route stability). Hand-authored skill files (`skills/radix-ng/SKILL.md` + its references) are not overwritten; generated files are in `.prettierignore` and CI-verified — **add any new generated output directory to `.prettierignore`**, or lint-staged reformats the JSON shards and CI reports drift.
...
- `composite`, `focus-scope`, `popper` — ordered composite item registration / roving focus, focus trapping, positioning. Use `RdxCompositeList` when a primitive only needs DOM-order item metadata without roving focus. **Content projection (ADR 0001):** items resolve their root/list through the _declaring_ injector tree, so an item projected into a `List` declared in a wrapper's own template can't inject it. `RdxCompositeItemOwner` bridges that — the primitive republishes the List's contexts as signals on its Root context and provides an owner on each item part (`tabs-composite-item-owner.ts`, `navigation-menu-composite-item-owner.ts`). An owner wins over injection even while its signals are `null`, so the Root must stay on the wrapper's _host_, and nesting a second composite root inside such a primitive requires making the owner conditional. **Thin positioners (ADR 0012):** every `<name>-positioner.ts` **`extends RdxPopperContentWrapper`** rather than composing it via `hostDirectives` — don't re-declare the ~11 positioning inputs (they're inherited; re-declaring re-introduces the doc-vs-runtime drift bug). Use `providers: [...provideRdxPopperContentWrapper(SelfClass), provideRdxPopperContentConfig({ … })]` (the helper re-wires the `useExisting` alias + context since Angular doesn't inherit a base directive's `providers`); per-primitive defaults live **only** in the config provider. The wrapper emits the unified `--anchor-*`/`--available-*`/`--positioner-*`/`--transform-origin` vars + `data-side`/`data-align`/`data-anchor-hidden` for everyone; `legacyPopperVars('<name>')` supplies the deprecated `--radix-<name>-*` aliases for one release. **`z-index` belongs on the positioner** (the wrapper no longer copies the popup's z, so a positioner no longer requires an inner `RdxPopperContent`). Behavior subclasses (grace-area, cursor-follow, tokens) add their logic and call `super()` — use a distinct `ElementRef` field name (e.g. `containerRef`), not `elementRef`, to avoid clobbering the base's.
- `dismissable-layer` — outside-dismiss (Escape / pointer-down-outside / focus-outside) as the `RdxDismiss` capability (`RdxDismissProps`), attached to the shared floating tree (ADR 0015). **Gotcha:** the focus-outside check is **async** (defers two microtasks before deciding), so an element that takes focus _outside_ a layer and then opens it can be dismissed a tick later. Mark such elements **"inside"** so focus/pointer on them won't dismiss — via `RdxFloatingInsideElement`, or (for triggers) the shared trigger registry the root exposes as `registerTrigger`, which the floating tree reads as inside. This replaced the removed process-global layer stack + `rdxDismissableLayerBranch` / `RdxDismissableLayersContextToken.branches` and the `disableOutsidePointerEvents` body toggle (modal pointer-blocking is now a cut-out `RdxInternalBackdrop`; ADR 0016/0017). Example: a menu trigger the menubar focuses before opening a sibling popup.
...
- `isMouseWithinBounds(event, element)` / `getPseudoElementBounds(element)` — whether a release happened on an element, counting a `::before`/`::after` that renders past it and a 5px drift tolerance. Used by the menu trigger's `cancel-open` guard so a slightly-off release does not close the menu it just opened
- `useInitialLiveRegionText(host)` — makes a polite live region that already has text when it mounts announce it (an invisible word joiner for 200ms), which Safari's VoiceOver otherwise misses. Wired into combobox status/empty and autocomplete empty; no-op on iOS and the server
