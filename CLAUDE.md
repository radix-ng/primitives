...
- **`@angular/cdk` is being phased out.** Do not add new CDK imports. CDK Dialog/Overlay, `cdk/portal`, and `CdkTrapFocus` are already fully replaced with own implementations (so consumers no longer need `@angular/cdk/overlay-prebuilt.css`). Remaining runtime usage to replace over time: `LiveAnnouncer` (`cdk/a11y`) and `Platform` (`cdk/platform`). For unique IDs use `injectId('rdx-…-')` (or the `RdxIdGenerator` service) from `@radix-ng/primitives/core` — the own replacement for CDK's `_IdGenerator`. The `BooleanInput` / `NumberInput` type aliases also now live in `core` — use those, not `cdk/coercion`. The `Direction` (`cdk/bidi`) type-only import is the last low-priority alias left. See the full breakdown in `.claude/skills/project-knowledge/references/architecture.md`.
