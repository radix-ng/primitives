...
- Status: Accepted (P0–P5 done — shared engine, both roots thin, parts parity; P6 (native form) next)
- Date: 2026-06-13
...
| 3   | High   | Missing Root API / native-form contract: `name`, `form`, `itemToStringValue`, `defaultInputValue`, `autoComplete`, controlled `filteredItems`, `grid`, `inline`, `locale`; `isItemEqualToValue` vs our `by`; `readOnly` vs our `readonly`; `filter(text)` vs `filter(itemValue, query, itemToString)` | Parity             | grid→**P2 done**, inline→P3, filter signature `(itemValue, query, itemToString?)`→**P1 done**, renames `readonly`→`readOnly` / `by`→`isItemEqualToValue`→**P1 done**, native-form (`name`/`form`)→P6                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- **P6: native-form contract.** `name`/`form`/`itemToStringValue` so a Combobox submits via a plain
  HTML form (Finding 3) — a hidden-input adapter; its own design.
...
Suggested next batch: **#4–#5** (visible inline-completion divergence) and **#6** (real perf on large
lists); the rest are lower-value polish.
