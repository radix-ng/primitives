...
| checkbox / collapsible / tabs presences | **unchanged** (docs-only improvement)                                         | inline presence, no portal — teleporting them would be wrong (content must stay in flow). Their "extra tag" is already solvable today: any `ng-template[x]` directive supports `*` microsyntax, e.g. `<div *rdxCollapsiblePanelPresence rdxCollapsiblePanel>` — showcase this in their anatomy docs. Collapsible panel additionally has `keepMounted` for the no-unmount path. Accordion (composes collapsible) gets the same for free |
